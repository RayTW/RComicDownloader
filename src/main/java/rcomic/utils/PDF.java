...
public class PDF {
	 ...
	 * @throws Exception
	 */
	public static boolean comicFolderToPDF(Rectangle pageSize, String sourceFolderPath, ArrayList<String> pathList,
			String targetPdfPath) throws Exception {
		boolean result = false;
		Document document = new Document(pageSize);

		PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(targetPdfPath));
	...
	public static boolean comicFolderToPDF(Rectangle pageSize, String sourceFolderPath, String targetPdfPath) {
		boolean result = false;
		Document document = new Document(pageSize);
		...
		try {
			...
			if (f.isDirectory()) {
				...
				for (File file : list) {
					...
					if (file.isFile() && (absolutePath.endsWith("jpg") || absolutePath.endsWith("jpeg"))) {
...
class HeaderFooter extends PdfPageEventHelper {
	...
	public void onEndPage(PdfWriter writer, Document document) {
		...
		ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_MIDDLE,
				new Phrase(Integer.toString(pageCount), mFont), center, 0 + 10, 0);
